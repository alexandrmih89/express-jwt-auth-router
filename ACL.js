...
const combine = require('./util/combine');
require('./util/dotenv');
...
function signToken(req, type, expiresIn) {
  const { id, provider, roles, device, iat, ...user } = req.user;
  ...
  //TODO: api?
  return jwt.sign({ ...user, id, provider, roles, device, type }, jwtSecret, expiresIn ? { expiresIn } : {});
}
